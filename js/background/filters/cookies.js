var cookieFilter = {
  ...
  cookie: {
    ...
    check: {
      ...
      valueMeaningfull: function (tabId, cookie) {
        if (formatConversion.isMeaningfulData(cookie.value) !== false) {
      ...
      valueExtractable: function (tabId, cookie) {
        let value = formatConversion.extractRecursively(cookie.value)
        if (value !== cookie.value && formatConversion.isMeaningfulData(value) !== false) {
  ...
  domain: {
    ...
    getDomainName: function (domain) {
      ...
      let regexVar = new RegExp(/([^.]*\.(([^.]{0,2}\.[^.]{0,2})|([^.]*)))$/g)
      let result = domain.match(regexVar)
